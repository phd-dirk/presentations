...
\note[itemize]{
  ...
  \item Marked the perturbative critical region with a grey background
...
\end{frame}
\note[itemize]{
  \item Strong coupling constant from tau decays
  ...
  \item Table shows produced mesons
  \item Use duality ansatz: theoretically quark-gluon picture, experimentally measure hadrons
  \item Duality is not always valid (Duality violations)
}
...
\end{frame}
\note[itemize]{
\item Circumvent the positive real axis by Cauchy's theorem
...
\item The blue line integral is experimentally accessible
...
\item Apply Cauchy's theorem to get theoretical integral moment
\item Note: Moments depend on \(\omega\) and \(s_0\), we only take part of the data into account
...
\begin{frame}
  ...
  \item The correlator is approximated by the operator product expansion
    \begin{equation}
      \Pi \to \Pi^{OPE}(s) = \sum_D \frac{1}{(-s)^{D/2}} \sum_{\dim \mathcal{O} = D} C_{D}(-s,\mu)\langle \mathcal{O}(\mu) \rangle
      \equiv \sum_{k=0}^\infty \frac{C_{2k}(s)}{(-s)^k}
    \end{equation}
  \item \(C_{D}\) are the Wilson coefficients, which can be calculated perturbatively
  \item \(\mathcal{O}\) are higher dimensional operators, e.g. \(D=4\)
    \begin{itemize}
      \item Quark condensate: \(m\langle \overline{q}{q} \rangle\)
      \item Gluon condensate: \(G_a^{\mu\nu}G^{a}_{\mu\nu}\)
    \end{itemize}
  \item The term with \(D = 0\) corresponds to the perturbative contribution
  \end{itemize}
\end{frame}
\note[itemize]{
\item QCD vacuum contains NPT contributions
\item Approximate correlator with OPE
\item The OPE separates short distances (high energies/ PT) from long distances
  (NPT)
\item Short distances \(\Rightarrow\) Wilson coefficients calculated by Feynman diagrams
\item Long distances \(\Rightarrow\) vacuum expectation value of higher dimensional operators
...
\item This is an ansatz, but cannot be derived from first principles
...
\item Method consists of the Borel transform and Borel integral
\item Beneke and Jamin (2008) modelled the Adler function
\item Follow method of Beneke and Jamin to use BS in fits to test validity of FOPT
}
...
\begin{frame}
  ...
  \item Dimension four vacuum condensate contributions:
    ...
    \end{equation}
  \item Gluon condensate can be quoted as
    \begin{equation}
...
\item In our analysis we will neglect the dimension two contributions as we work
  in the chiral limit and their contributions are proportional to the quark
  masses }
...
\begin{frame}
  ...
  \begin{tabular}{ccccccc}
    ...
    & \(2.6\) & 4 & 0.3398(15) & -1.3(1.0) & -2.3(2.5) & 0.01  \\
    \bottomrule
  \end{tabular}
\end{frame}
\note[itemize]{
...
\begin{frame}
  ...
  \centering \resizebox{\textwidth}{!}{
    \begin{tabular}{cccccccc}
      \toprule
      weight & PT & \(\#s_0\)'s & \(\alpha_s(m_\tau^2)\) & \(\langle aGG \rangle_I\) & \(\rho^{(6)}\) & \(\rho^{(8)}\) & \(\chi^2/dof\)  \\
      \midrule
      ...
      \(1 - x\)           & BS & 7 & 0.3246(52) & -0.2262(59)  & \(0^*\)           & \(0^*\)   & 0.38 \\
      \((1 - x)^2\)       & BS & 7 & 0.3270(54) & -0.0254(61)  & -0.77(21)   & \(0^*\)   & 0.74 \\
      \((1 - x)^3\)       & BS & 8 & 0.3239(40) & -0.0212(42)  & -0.63(15)   & -0.74(29) & 0.46 \\
...
\begin{frame}
  ...
  \item Obtained values for the strong coupling:
    \begin{equation*}
      \begin{split}
        \alpha_s(m_\tau) &= 0.3268(44)(25) = 0.3268(51) \\
    ...
    \begin{equation*}
      \begin{split}
        \alpha_s(m_Z) &= 0.11886(53)(30)(5) = 0.11886(61) \\
