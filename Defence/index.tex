...
\usepackage[style=authortitle-comp, backend=biber,style=alphabetic ]{biblatex}
\usepackage{xpatch}
...
\note[itemize]{
  ...
  \item \(\tau \to \pi^-\nu_\tau\) is Cabbibo allowed, \(\tau \to K^- \nu_\tau\)
    is Cabbibo suppressed (\(\abs{V_{us}}^2 = (0.2)^2\))
}
...
\begin{frame}
  \frametitle{FOPT vs CIPT}
  \begin{itemize}
    \item  Perturbative FOPT and CIPT contributions (\(\alpha_s(m_\tau^2)=0.3186\)):
    \begin{align}
...
\item In our analysis we will neglect the dimension two contributions as we work
  ...
  masses
\item We work with the invariant gluon condensate, import because disagreement
}
