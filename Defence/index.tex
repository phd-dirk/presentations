...
\begin{frame}
  ...
  Inclusive Hadronic Tau Decay Ratio
  \begin{footnotesize}
    \begin{equation}
...
\end{frame}
\note[itemize]{
  \item It is common to rewrite the two-point function in terms of the Adler function.
...
\begin{frame}
  ...
  \begin{equation}
    \lim_{x^\mu \to 0} A(x)B(0) = \sum_n C_n(x)\mathcal{O}_n(0)
  \end{equation}
  % \begin{equation}
  ...
  \begin{equation}
    \begin{split}
    \Pi_{OPE}(s) &= \sum_{n=0,2,4,\dots} \frac{C_n \langle\Omega\vert \mathcal{O}_n (x) \vert\Omega\rangle}{(s)^{n/2}} \\
...
\note[itemize]{
  ...
  \item To do so we will describe the two-point function in terms of the
  ...
  \item The OPE separates short distances (high energies/ PT) from long distances (NPT)
  \item Short distances are given by the Wilson coefficients \(C_n(x-y)\),
    whereas the long distances are given by higher order operators \(\langle
    \Omega \vert \mathcal{O}_n(x) \rangle\).
  ...
  \item The Wilson coefficients can be calculated from Feynman diagrams, but the
    higher dimensional contributions have to be taken from NPT tools like
    lattice qcd or from our fits. We will determine values for the dimension six
    and eight operators
  \item The dimension zero contribution is the perturbative contribution,
    whereas the higher dimensional contributions are non-perturbative. We will
    deal with the PT contributions first before coming back to the NPT ones.
}
...
\begin{frame}
  ...
  \begin{equation}
    \Pi_V^{(1+0)}(s) = - \frac{N_c}{12 \pi^2} \sum_{n=0}^\infty a(\mu^2)^n \sum_{k=0}^{n+1} c_{n,k} L^k
  ...
  \begin{equation}
    D_V^{(1+0)}(s) = \frac{N_c}{12 \pi^2} \sum_{n=0}^\infty a(\mu^2)^n \sum_{k=1}^{n+1} k\, c_{n,k} L^{k-1}
  \end{equation}
\end{frame}
\note[itemize]{
  \item The two-point function can be expanded as a sum over different powers of
    the strong coupling
  \item We are working in the chiral limit. Consequently the two-point function
    of the \(V\) and \(A\) are identical. Thus from now on we will consider only
    the vector two-point function \(\Pi_V\)
  \item We can apply the derivative with respect to s to obtain the expansion of
    the Adler function
  \item The coefficients \(c_{n,k}\) are referred to as Adler function
    coefficients and can be calculated from Feynman diagrams
  \item We will see now that we do not have to calculate every single
    coefficient, but that the coefficients are internally related due to the RGE
}
...
\end{frame}
\note[itemize]{
  \item Let \(R(q, a_s, m)\) be a physical quantity, which is renormalisation
    scale invariant, then the derivative with respect to the renormalisation
    scale is zero
  \item From the RGE we can also define the \(\beta\) function, which is
    responsible for the running of the coupling
  \item For the expanded Adler function \(D_V^{(1+0)}(s)\) the RGE yields
    equation 19
  \item If we plug in the expansion of the Adler function in equation 19 we can
    relate the Adler coefficients
  \item Thus we only have to calculate Adler coefficients on the left side like
    \(c_{0,0}, c_{1,1}, \dots\) and Adler coefficients of higher indices are
    related to those
  \item The full series of the expanded Adler function is renormalisation scale
    invariant, but the Adler coefficients are only known up to fifth order.
    Consequently we have to cut off the series, which leaves us with a choice of
    fixing the invariant scale.
}
...
\begin{frame}
  ...
  \begin{small}
    ...
    \begin{tikzpicture}[>=triangle 45,font=\sffamily]
      \node (X) at (0,0) {};
      \node (Y) [below left=0.5cm and 3cm of X]  {};% 2cm below, 1cm to the left (optional)
      \node (Z) [below right=0.5cm and 3cm of X] {};
      \draw [semithick,->] (X) -- (Y);
      \draw [semithick,->] (X) -- (Z);
    \end{tikzpicture}
    \begin{columns}
      ...
      \begin{column}{.5\textwidth}
        ...
        \begin{ceqn}
          \begin{scriptsize}
            ...
            \end{equation}
            \begin{equation}
              J_n^a(m_\tau^2) \equiv \oint_{\abs{x}=1} \frac{\dif x}{x} \omega_J(x) a^n (-m_\tau^2 x)
            \end{equation}
          \end{scriptsize}
        \end{ceqn}
      \end{column}
...
\note[itemize]{
  ...
  \item Having the freedom to fix \(\mu\) leads to two different treatments of
    the PT contributions
  ...
  \item On the other hand CIPT fixed \(\mu \equiv -m_\tau^2 x\), which sums up
    the logarithms, but leaves us with a running coupling
...
\note[itemize]{
  ...
  \item The graph below has been taken from a paper of Beneke and Jamin who
    invested the topic
  \item here we see as the black dots the FOPT contribution, as the gray dots
    the CIPT contribution and as a straight line the Borel sum to which we will
    come in a minute  to which we will come in a minute  to which is used to sum
...
\end{frame}
\note[itemize]{
  \item The Borel summation is a summation method for divergent asymptotic
    series and should give us the best possible sum
  \item It consists of the Borel integral and the Borel transform, which we
    apply to the expansion of the Adler function
...
\end{frame}
\note[itemize]{
  \item Beneke and Jamin created a created the Borel model 
  \item The model depends on five parameters
  \item The Borel model should reproduce the exactly known result up to order
    fourth order of the Adler function
  \item Consequently the five parameters are obtained by matching the result
  \item This model is then used to sum the Adler function expansion
}
