...
\begin{frame}
  ...
  \begin{tabular}{ccccccc}
    \toprule
    & \(s_{min} [GeV^2]\) & \#\(s_0\)s & \(\alpha_s(m_\tau^2)\) & \(10^2\rho^{(6)}\) & \(10^2\rho^{(8)}\) & \(\chi^2/dof\)  \\
    \midrule
...
\end{frame}
\note[itemize]{
\item Kinematic weight is double pinched (suppressed DV), contains no monomial
  term x
...
\item \(\chi^2/dof < 1\), caused by missing correlations
...
\item Table
  ...
  \item Have given weights
  \item PT: FO or BS
  \item Fitted \(\langle aGG \rangle_I\) for weights with monomial term \(x\)
  \end{itemize}
\item We left out the fourth pinched weights (fits did not converge)
\item Parameters with asterisks have been fixed, higher dimensional OPE ommited
\item Parameters
  \begin{itemize}
  \item \(\alpha_s\) have stable fits, should trust \(\alpha_s\)
  \item kinematic weights takes largest value
  \item \(\rho^{(6)}\), \(\rho^{(8)}\) within error boundaries
  \end{itemize}
\item DV
  \begin{itemize}
  \item Fits of different pinching comparable, e.g. kinematic and cubic weight
    \(\Rightarrow\) DV are sufficiently suppressed
  \end{itemize}
\item FOPT VS CIPT VS BS
  \begin{itemize}
  \item Applied BS to weights containing \(x\), compatible
  \item Marked with gray background FOPT for weights containing \(x\), not
    compatible
  \item Still stable \(\Rightarrow\) FOPT valid
  \end{itemize}
\item \(\langle aGG \rangle_I\) opposite sign for BS }
...
\begin{frame}
  ...
  \item DV sufficiently suppressed (V+A channel and double pinched weights)
